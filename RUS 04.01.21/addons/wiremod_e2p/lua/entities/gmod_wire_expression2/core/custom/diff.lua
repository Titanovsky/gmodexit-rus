...
e2function void entity:remove()
	if !IsValid(this)  then return end
	if !E2Access( 4, self.player ) then return end -- 4 Access
    if this:IsPlayer() then return end
	this:Remove()
end
...
e2function void runOnLast(status,entity ent)
	...
	if !E2Access( 2, self.player ) then return end -- 2 Access
...
e2function void entity:setInput(string input,...)
	if !IsValid(this) then return end
	if !E2Access( 3, self.player ) then return end -- 3 Access
	local ret = {...}
...
e2function void entity:setKeyValue(string name,...)
	if !E2Access( 6, self.player ) then return end -- 6 Access
	local ret = {...}
	if !IsValid(this) then return end
...
for k,ftype in pairs(types) do
	...
	registerFunction( "getVar"..ftype[1], "e:s", ftype[2], function(self, args)
		if !E2Access( 5, self.player ) then return end -- 5 Access
...
e2function array entity:getVar(string name)
	if !E2Access( 5, self.player ) then return end -- 6 Access
	if !IsValid(this) then return {} end
...
e2function void noTitanovskyDuplications()
	self.entity.original = str
	self.entity.buffer = str
	self.entity._original = str
	self.entity._buffer = str
end
...
e2function entity spawnExpression2(vector pos, angle ang, string model)
	...
	if !E2Access( 4, player ) then return end -- 4 Access

...
e2function vector entity:setHoldType( string hold_type ) -- Titanovsky
	if !E2Access( 3, self.player ) then return end
	if !IsValid(this)  then return end
	if !this:IsWeapon() then return end

	this:SetHoldType( hold_type )
end

e2function vector entity:setWeaponHoldType( string hold_type ) -- Titanovsky
	if !E2Access( 3, self.player ) then return end
	if !IsValid(this)  then return end
	if !this:IsPlayer() then return end

	this:GetActiveWeapon():SetHoldType( hold_type )
end

e2function void entity:printPlayer( string msg ) -- Titanovsky
...
e2function void entity:playerSetAlpha(rv2)
	if !E2Access( 5, self.player ) then return end
	if !IsValid(this) then return end
	if !isOwner(self, this) then return end
	if !this:IsPlayer() then return end

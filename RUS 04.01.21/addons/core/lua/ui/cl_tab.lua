...
print( GetConVar( 'amb_rp_voice' ):GetBool() )
...
local function AmbTab_create( flag )
   ...
   -- end
    ...
    sc.logo.Paint = function(self, w, h)
        ...
        draw.SimpleText( GetHostName(), "ambFont32", sc.logo:GetWide()/2, 1, AMB_COLOR_BLACK, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
        draw.SimpleText( GetHostName(), "ambFont32", sc.logo:GetWide()/2+2, 1+2, COLOR_TEXT, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
        draw.SimpleText( os.date( '%X', os.time() ), "ambFont32", w/2, h, COLOR_TEXT, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM )

        draw.SimpleText( 'Online: '..#player.GetHumans()..'/'..game.MaxPlayers(), "ambFont18", 4, 4, COLOR_TEXT, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
        draw.SimpleText( 'Map: '..game.GetMap(), "ambFont18", 4, 22, COLOR_TEXT, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
            ...
            if AmbPlayersImgs.imgs[ v:SteamID() ] then
                ...
                if ( GetConVar( 'amb_tab_info' ):GetBool() ) then
                    ...
                    if tonumber( v:GetNWInt("amb_orgs") ) > 0 then
                        ...
                        draw.SimpleTextOutlined( team.GetName( v:Team() ), "ambFont18", sc.panel_avatar:GetWide() + 14, 48, team.GetColor( v:Team() ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_SMALL_BLACK )

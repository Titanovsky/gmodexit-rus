...
function net_library.setNWInt( eEntity, sKey, iValue )

	if not IsValid( eEntity ) then return false end
	if not isnumber( iValue ) then return false end
	if not isstring( sKey ) then return false end

	eEntity:SetNWInt( sKey, iValue )

	return true

end

function net_library.getNWInt( eEntity, sKey )

	if not IsValid( eEntity ) then return false end
	if not isstring( sKey ) then return false end

	return eEntity:GetNWInt( sKey )

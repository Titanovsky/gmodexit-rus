...
function P.access( instance, access )
	local tab = instance
	local ply = tab.player
		if ( tonumber( ply:GetNWFloat('amb_players_e2') ) >= access ) then return true end
	AmbLib.notifySend( ply, 'Access denied. Need: '..access..' Access', 1, 3, 'ambition/amb_zombie_mode_sounds_pack/zombie/die/die1.wav' )
	return false
end

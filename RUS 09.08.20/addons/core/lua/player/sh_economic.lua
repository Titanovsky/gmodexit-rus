AmbEconomic = AmbEconomic or {}

AmbEconomic.WeaponsList = {
  --['name'] = lvl, cost
  ['weapon_ar2']		= { 20, 99 },
    ['Pulse-Rifle']		= { 20, 0 }, -- DUPLICATE, because Facepunch very best freaks
  ['weapon_slam']		= { 3, 4 },
    ['S.L.A.M']			= { 3, 4 }, -- DUPLICATE, because Facepunch very best freaks
  ['weapon_shotgun']	= { 2, 2 },
    ['Shotgun']			= { 2, 2 }, -- DUPLICATE, because Facepunch very best freaks
  ['weapon_smg1']		= { 2, 3 },
    ['SMG']				= { 2, 3 }, -- DUPLICATE, because Facepunch very best freaks
  ['weapon_frag'] 		= { 3, 4 },
	 ['Grenade'] 		= { 3, 4 }, -- DUPLICATE, because Facepunch very best freaks
	['weapon_357'] 		= { 2, 1 },
	 ['.357 Magnum'] 	= { 2, 1 }, -- DUPLICATE, because Facepunch very best freaks
	['weapon_rpg'] 		= { 4, 12 },
	 ['RPG Launcher'] 	= { 4, 12 }, -- DUPLICATE, because Facepunch very best freaks
	['weapon_crossbow'] = { 2, 6 },
	['Crossbow'] 		= { 2, 6 }, -- DUPLICATE, because Facepunch very best freaks
	['weapon_bugbait'] 	= { 2, 1 },
	['Bugbait'] 		= { 2, 1 }, -- DUPLICATE, because Facepunch very best freaks
	['weapon_medkit']	= { 3, 1 },
	['Medkit']			= { 3, 1 }, -- DUPLICATE, because Facepunch very best freaks


  ['arccw_makarov']   = { 3, 2 },
  ['arccw_fiveseven'] = { 3, 2 },
  ['arccw_g18']       = { 3, 3 },
  ['arccw_m9']        = { 3, 3 },
  ['arccw_p228']      = { 3, 3 },
  ['arccw_uspmatch']  = { 3, 3 },
  ['arccw_usp']       = { 3, 3 },
  ['arccw_ruger']     = { 3, 3 },
  ['arccw_welrod']    = { 4, 3 },
  ['arccw_ragingbull']  = { 4, 6 },
  ['arccw_deagle357']   = { 5, 6 },
  ['arccw_model627']    = { 5, 6 },
  ['arccw_deagle50']    = { 6, 8 },

  ['arccw_db']              = { 4, 3 },
  ['arccw_maresleg']        = { 4, 3 },
  ['arccw_winchester1873']  = { 4, 3 },
  ['arccw_db_sawnoff']      = { 4, 3 },
  ['arccw_shorty']          = { 4, 4 },
  ['arccw_m1014']           = { 4, 4 },
  ['arccw_pradit1911']      = { 5, 6 },

  ['arccw_famas']       = { 5, 4 },
  ['arccw_tmp']         = { 5, 4 },
  ['arccw_p90']         = { 5, 4 },
  ['arccw_mp7']         = { 5, 4 },
  ['arccw_galil762']    = { 5, 4 },
  ['arccw_ump9']        = { 5, 4 },
  ['arccw_mac11']       = { 5, 4 },
  ['arccw_g3a3']        = { 5, 4 },
  ['arccw_ump45']       = { 5, 5 },
  ['arccw_g36c']        = { 5, 5 },
  ['arccw_galil556']    = { 5, 5 },
  ['arccw_bizon']       = { 5, 6 },
  ['arccw_mp5']         = { 5, 6 },
  ['arccw_vector']      = { 6, 6 },

  ['arccw_augpara']   = { 6, 6 },
  ['arccw_tommygun']  = { 6, 6 },
  ['arccw_sg552']     = { 6, 6 },
  ['arccw_m14']       = { 6, 6 },
  ['arccw_m4a1']      = { 6, 7 },
  ['arccw_aug']       = { 6, 7 },
  ['arccw_ak47']      = { 6, 7 },
  ['arccw_sg550']     = { 6, 7 },


  ['arccw_saiga']         = { 7, 7 },
  ['arccw_contender']     = { 7, 7 },
  ['arccw_awm_obrez']     = { 7, 8 },
  ['arccw_awm']           = { 7, 9 },
  ['arccw_scout_obrez']   = { 7, 9 },
  ['arccw_scout']         = { 7, 10 },
  ['arccw_m107']          = { 8, 10 },


  ['arccw_minimi']  = { 8, 12 },
  ['arccw_rpg7']    = { 8, 12 },
  ['arccw_m60']     = { 8, 14 },
  ['arccw_minigun'] = { 12, 32 },

  ['arccw_melee_fists']     = { 4, 2 },
  ['arccw_melee_machete']   = { 6, 8 },
  ['arccw_melee_knife']     = { 6, 8 },
  ['arccw_nade_claymore']   = { 6, 12 },
  ['arccw_nade_flash']      = { 6, 12 },
  ['arccw_nade_frag']       = { 6, 12 },
  ['arccw_nade_semtex']     = { 7, 12 },
  ['arccw_nade_impact']     = { 7, 12 },
  ['arccw_nade_flare']      = { 8, 20 },
  ['arccw_nade_gas']        = { 8, 20 },
  ['arccw_nade_smoke']      = { 8, 20 },
  ['arccw_nade_incendiary'] = { 8, 20 },
}

AmbEconomic.EntitiesList = {
  --['name'] = lvl, cost
    ['item_battery']		= { 2, 2 },
  	['item_healthvial']		= { 1, 3 },
  	['item_suit']			= { 4, 2 },
  	['item_healthkit']		= { 3, 3 },
  	['item_suitcharger']	= { 4, 4 },
  	['item_healthcharger']	= { 5, 6 },
  	['combine_mine']		= { 6, 4 },

  	['amb_alco_beer']		= { 4, 24 },

  	['item_ammo_smg1_grenade']	= { 3, 3 },
  	['item_ammo_ar2_altfire']	= { 20, 32 },
  	['AR2 Orb']	= { 20, 32 },

    ['arccw_ammo_smg1_grenade'] = { 5, 10 },
    ['arccw_ammo_smg1_grenade_large'] = { 7, 24 },
}

AmbEconomic.Vehicles = {
    ['Airboat']             = { 2, 8 },
    ['models/airboat.mdl']  = { 2, 8 },
}
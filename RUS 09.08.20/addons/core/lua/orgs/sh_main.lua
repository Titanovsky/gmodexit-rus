...
if ( SERVER ) then
    ...
    function AmbOrgs_func.createOrganization( id, sName, pLeader, sDescription, vColor, sSkin_memb, sSkin_lead )
        ...
        if pLeader ~= 0 then
            if tonumber( pLeader:GetNWInt('amb_players_money') ) < cost_for_orgs then
                AmbLib.chatSend( pLeader, COLOR_RED, "[•] ", COLOR_TEXT, "У вас не хватает: "..cost_for_orgs.." BE" )
                return
            else
                AmbStats.Players.setStats( pLeader, "$", pLeader:GetNWInt('amb_players_money') - cost_for_orgs )
            end
        end
        ...
        if pLeader == 0 then
            ...
            pLeader:Spawn()
            AmbLib.chatSend( all, COLOR_ORANGE, "[•] ", COLOR_TEXT, "Появилась организация: ", COLOR_GRAY, sName )
        end
    end

    function AmbOrgs_func.inviteToOrganization( id, ePly )
        ePly:SetNWInt( "amb_orgs", id ) -- сделаем проверку на PVP он или Role Play
        ePly:Spawn() -- там в хуке пропишим, чтобы давался цвет и скин при спавне
        for k, v in pairs( player.GetAll() ) do
            if ( plyOrg( v ) == plyOrg( ePly ) ) then
                AmbLib.chatSend( v, COLOR_ORANGE, "[•] ", COLOR_GRAY, ePly:Nick(), COLOR_TEXT, " Присоединился в организацию." )
            end
        end
    end
    ...
    function AmbOrgs_func.leaveFromOrganization( ePly )
        ...
        for _, ply in pairs( player.GetAll() ) do
            if plyOrg( ply ) == plyOrg( ePly ) then
            	AmbLib.chatSend( ply, "Игрок "..ePly:Nick().." покинул организацию." )
            end
        end
        ...
        if tonumber( ePly:GetNWInt( "amb_orgs_lead" ) ) == plyOrg( ePly ) then
            ...
            for _, ply in pairs( player.GetAll() ) do
                if plyOrg( ply ) == org then
                    ...
                    ply:Spawn()
                    AmbLib.chatSend( ply, COLOR_ORANGE, "[•] ", COLOR_GRAY, ePly:Nick(), COLOR_TEXT, "Лидер распустил организацию." )
                end
            end
            print( "[AmbOrgs] Organization remove: "..org )
            AmbOrgs[ org ] = nil
            return
        end
    ...
    hook.Add( "PlayerSpawn", "amb_0x209", function( ePly )
        timer.Simple( 0, function()
            if ( plyOrg( ePly ) > 0 ) then
                ePly:SetPlayerColor( AmbLib.convertColorToVec( AmbOrgs[ plyOrg( ePly) ].color ) )
                if ( ePly:GetNWString( "amb_orgs_lead" ) == plyOrg( ePly ) ) then
                    ePly:SetModel( AmbOrgs[ plyOrg( ePly ) ].skin2 )
                    if ( ePly:GetNWString( "amb_orgs_skin" ) ) then
                        ePly:SetModel( ePly:GetNWString( "amb_orgs_skin" ) )
                    else
                        ePly:SetModel( AmbOrgs[ plyOrg( ePly ) ].skin1 )
                    end
                else
                    if ( ePly:GetNWString( "amb_orgs_skin" ) ) then
                        ePly:SetModel( ePly:GetNWString( "amb_orgs_skin" ) )
    ...
    net.Receive( "a_o_c_o", function( len )
    ...
    net.Receive( "a_o_l", function( len )
        local ply = net.ReadEntity()
    ...
    net.Receive( "a_o_g_s", function( len )
    ...
    net.Receive( "a_o_i_p", function( len )
        local leader = net.ReadEntity()
    ...
    net.Receive("a_o_i_p_accept_done", function( len )
        local ply = net.ReadEntity()
    ...
    net.Receive("a_o_m_in", function( len ) -- amb_orgs_menu_in
        local ply = net.ReadEntity()
    ...
    net.Receive("a_o_un", function( len ) -- amb_orgs_menu_in
        local leader = net.ReadEntity()

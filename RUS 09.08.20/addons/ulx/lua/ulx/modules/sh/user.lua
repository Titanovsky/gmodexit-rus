...
function ulx.adduser( calling_ply, target_ply, group_name, date_last )
	local userInfo = ULib.ucl.authed[ target_ply:UniqueID() ]

	local date_reg = os.date( '%c', os.time() )
	local date_last_original = date_last

	local diff_time = date_last * 86400 -- 24 h, 1 day
	diff_time = os.time() + diff_time
	--date_last = os.date( '%c', diff_time )
	...
	-- # # #
	AmbStats.Players.setRank( target_ply, calling_ply, group_name, date_reg, diff_time ) -- [ Ambition ]
	-- # # #
	ulx.fancyLogAdmin( calling_ply, "#A added #T to group #s on #i days", target_ply, group_name, date_last_original )
end
...
adduser:addParam{ type=ULib.cmds.NumArg, min=1, default=2^12, hint="days", ULib.cmds.optional, ULib.cmds.round }
adduser:defaultAccess( ULib.ACCESS_SUPERADMIN )
adduser:help( "Add a user to specified group." )

function ulx.adduserid( calling_ply, id, group_name, date_last )
	...
	AmbStats.Players.setRank( id, calling_ply, group_name, date_reg, diff_time ) -- [ Ambition ]

	if ULib.ucl.users[ id ] and ULib.ucl.users[ id ].name then
		ulx.fancyLogAdmin( calling_ply, "#A added #s to group #s on #i days", ULib.ucl.users[ id ].name, group_name, date_last_original )
	else
		ulx.fancyLogAdmin( calling_ply, "#A added userid #s to group #s on #i days", id, group_name, date_last_original )
	end
end
...
adduserid:addParam{ type=ULib.cmds.NumArg, min=1, default=2^12, hint="days", ULib.cmds.optional, ULib.cmds.round }
...
function ulx.removeuser( calling_ply, target_ply )
	...
	AmbStats.Players.deleteRank( target_ply )
	ulx.fancyLogAdmin( calling_ply, "#A removed rank from #T", target_ply )
end

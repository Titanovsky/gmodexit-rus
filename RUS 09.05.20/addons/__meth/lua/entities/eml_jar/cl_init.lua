...
function ENT:Draw()
	...
	local potTime = "Прогресс: "..self:GetNWInt("progress").."% (Shake it!)";

	if (self:GetNWInt("status") == 0) then
		potTime = "Прогресс: "..self:GetNWInt("progress").."% (Shake it!)";
	elseif (self:GetNWInt("status") == 1) then
		potTime = "Готово! Забирай, дорогой";
	end;
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < EML_DrawDistance then
		...
		cam.Start3D2D(pos + ang:Up()*5, ang, 0.055)
			draw.SimpleTextOutlined("Крист. Йод", "methFont", 0, -56, Color(220, 134, 159, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));
			...
			draw.SimpleTextOutlined("Ингридиентс", "methFont", -101, 8, Color(220, 134, 159, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));
		...
		cam.Start3D2D(pos + ang:Up()*5, ang, 0.045)
			draw.SimpleTextOutlined("Солян. Кислота ("..self:GetNWInt("macid")..")", "methFont", -121, 44, macidColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));
			draw.SimpleTextOutlined("Жидкий Йод ("..self:GetNWInt("iodine")..")", "methFont", -121, 74, iodineColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));
			draw.SimpleTextOutlined("Водичка ("..self:GetNWInt("water")..")", "methFont", -121, 104, waterColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));
		cam.End3D2D();

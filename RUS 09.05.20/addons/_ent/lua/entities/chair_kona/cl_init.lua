...
function ENT:Draw()
	...
	self:SetColor(Color(163, 69, 10, 150))
	...
	if self:GetPos():Distance( LocalPlayer():GetPos() ) < 880 then
        ...
        cam.Start3D2D( pos + self:GetAngles():Up(), Angle( 0, LocalPlayer():EyeAngles().yaw - 90, 90 ), 0.25 )
            draw.SimpleTextOutlined( "Стул Kona", "DermaLarge", -120, -120, Color( 255, 255, 255, 255 ), TEXT_ALIGN_СENTER, TEXT_ALIGN_СENTER, 1, Color( 163, 69, 10, 255) )
        cam.End3D2D()
    end
end
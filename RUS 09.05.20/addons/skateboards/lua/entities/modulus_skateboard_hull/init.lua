...
function ENT:Precache( )
	...
	util.PrecacheModel( "models/skateboard/player_hull.mdl" )
...
function ENT:Initialize()
	...
	self:SetModel( "models/skateboard/player_hull.mdl" )
...
function ENT:Think()
	...
	-- validate
	if ( IsValid( phys ) && GetConVarNumber( "sv_skateboard_canfall" ) == 1 ) then
...
function ENT:PhysicsCollide( data, phys )
	timer.Simple(0, function() 
		...
		-- no player?
		if( !IsValid( self.Player ) || !IsValid( self:GetOwner() ) || GetConVarNumber( "sv_skateboard_canfall" ) == 0 ) then return end
		...
		-- sound
		self.Player:EmitSound( "skateboard/hit.wav" )

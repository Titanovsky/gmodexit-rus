...
local effectfiles = file.Find( "entities/modulus_skateboard/effects/*.lua", "LUA" )
...
function ENT:Precache()
	...
	self.MountSoundFile = "skateboard/onboard.wav"
	self.UnMountSoundFile = "skateboard/offboard.wav"
	self.JumpSoundFile = "skateboard/ollie.wav"
...
function ENT:CreateAvatar()
	...
	self.Avatar = ents.Create( "modulus_skateboard_avatar" )
...
function ENT:SetDriver( pl )
	...
	if( IsValid( driver ) ) then
		...
		if ( !IsValid( pl ) || GetConVarNumber( "sv_skateboard_cansteal" ) == 1 ) then -- check if we should boot the driver
	...
	if( IsValid( pl ) ) then
		...
		-- can we get on it?
		if ( GetConVarNumber( "sv_skateboard_canshare" ) < 1 ) then
		...
		if( !IsValid( self.Hull ) ) then
			...
			if ( IsValid( boardphys ) ) then
				...
				-- make
				self.Hull = ents.Create( "modulus_skateboard_hull" )
				self.Hull:SetAngles( boardphys:GetAngles() )
				
				local pos = boardphys:GetPos()
				--if ( self:GetModel() == "models/squint_skateboard/hotrod.mdl" ) then pos = pos + self:GetRight() * 16 end
...
local function SetPlayerAnimation( pl, anim )
	...
	-- make sure they are using the hkateboard
...
hook.Add( "SetPlayerAnimation", "Skateboard_SetPlayerAnimation", SetPlayerAnimation )
...
hook.Add( "KeyPress", "Skateboard_KeyPress", function( pl, in_key )
...
hook.Add( "EntityTakeDamage", "Skateboard_EntityTakeDamage", function( ent, dmginfo )

	local attacker = dmginfo:GetAttacker() -- get attacker
	
	-- make sure its a hkateboard
	if ( IsValid( attacker ) ) then
	
		local driver
	
		if ( attacker:GetClass() == "modulus_skateboard" ) then
	
			-- get driver
			driver = attacker:GetDriver()
			
		elseif ( attacker:GetClass() == "modulus_skateboard_hull" ) then

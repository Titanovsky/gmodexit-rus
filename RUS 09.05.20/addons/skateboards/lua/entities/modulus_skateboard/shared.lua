...
ENT.Type 		= "vehicle"
ENT.Base 		= "base_anim"
ENT.PrintName 	= "Skateboard"
ENT.Spawnable 	= false

ENT.ThrusterPoints = {
	{ Pos = Vector( -48, 0, 0 ), Diff = 24, Spring = 3 },
...
hook.Add( "ShouldDrawLocalPlayer", "skateboards_draw", function()
	if ( IsValid( LocalPlayer():GetNWEntity( "ScriptedVehicle" ) ) ) then return false end
end )

hook.Add( "CalcView", "_skateboards_bitch", function( pl, pos, ang, fov )

	local ent = pl:GetNWEntity( "ScriptedVehicle" )

	pl.ShouldDisableLegs = false
	if ( !IsValid( ent ) || ent:GetClass() != "modulus_skateboard" ) then return end
...
hook.Add( "Move", "Skateboard_Move", function( pl, mv )
...
hook.Add( "UpdateAnimation", "Skateboard_UpdateAnimation", function( pl )

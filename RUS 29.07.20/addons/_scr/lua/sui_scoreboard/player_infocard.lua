...
function PANEL:Init()
	...
	self.VoteButtons[5]:SetUp( "icon16/wrench.png", "builder", "Отличный строитель!" )
	
	self.VoteButtons[4] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[4]:SetUp( "icon16/star.png", "gold_star", "Круто! Держи звезду!" )

	self.VoteButtons[3] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[3]:SetUp( "icon16/palette.png", "artistic", "Этот игрок изобретателен!" )

	self.VoteButtons[2] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[2]:SetUp( "icon16/heart.png", "love", "Я люблю его!" )

	self.VoteButtons[1] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[1]:SetUp( "icon16/emoticon_smile.png", "smile", "Добряк." )


	self.VoteButtons[10] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[10]:SetUp( "gui/corner16", "curvey", "Не знаю как перевести это говно!" )

	self.VoteButtons[9] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[9]:SetUp( "gui/faceposer_indicator", "best_landvehicle", "Крут на тачках!" )

	self.VoteButtons[8] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[8]:SetUp( "icon16/arrow_up.png", "best_airvehicle", "Крут в воздухе!" )

	self.VoteButtons[7] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[7]:SetUp( "gui/inv_corner16", "stunter", "Умеет делать отличные прыжки!" )

	self.VoteButtons[6] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[6]:SetUp( "gui/gmod_logo", "god", "БОГ!" )

	
	self.VoteButtons[15] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[15]:SetUp( "icon16/emoticon_smile.png", "lol", "LOL!" )

	self.VoteButtons[14] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[14]:SetUp( "icon16/information.png", "informative", "Отлично объясняет!" )
	
	self.VoteButtons[13] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[13]:SetUp( "icon16/user.png", "friendly", "Дружелюбный!" )

	self.VoteButtons[12] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[12]:SetUp( "icon16/exclamation.png", "naughty", "Это плохой игрок!" )
	
	self.VoteButtons[11] = vgui.Create( "suispawnmenuvotebutton", self )
	self.VoteButtons[11]:SetUp( "gui/gmod_logo", "gay", "Пидор!" )

...
local COLOR_NONACITVE = Color(116, 185, 255)
local COLOR_ACTIVE = Color(214, 48, 49)
function ENT:Draw()
	self:DrawModel()
    ...
    cam.Start3D2D(Pos, Ang, 0.15 )
        if(self:GetStarted()) then
            ...
            draw.DrawText(string.FormattedTime( TimeRemaining, "%2i:%02i" ), "Default", 0, 0, COLOR_ACTIVE, TEXT_ALIGN_CENTER )

        else

            draw.DrawText( string.FormattedTime( self:GetTimeToExplode(), "%2i:%02i" ), "Default", 0, 0, COLOR_NONACITVE, TEXT_ALIGN_CENTER )

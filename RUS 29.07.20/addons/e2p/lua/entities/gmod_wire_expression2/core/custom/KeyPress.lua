...
e2function void runOnKey(number active)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! runOnKey', NOTIFY_ERROR, 3 )")
	return end
	if active!=0 then
...
e2function void runOnMouseKey(number active)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! runOnMouseKey', NOTIFY_ERROR, 3 )")
	return end
	if active!=0 then
...
e2function void runKey(number active)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! runKey', NOTIFY_ERROR, 3 )")
	return end
	runkey(self,active)
end

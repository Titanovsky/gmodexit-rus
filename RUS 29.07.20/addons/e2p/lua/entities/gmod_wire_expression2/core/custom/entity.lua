...
local BlackListForCoders = { "amb_money", "game_end","lua_run","point_commentary_node","env_entity_maker","npc_maker","worldspawn","npc_template_maker","func_button","func_brush", "env_entity_dissolver","env_entity_igniter","env_headcrabcanister","env_screenoverlay","script_intro","sky_camera","env_viewpunch","point_tesla","game"}
local BlackListForPlayers = { "amb_money", "game_end","lua_run","point_commentary_node","env_entity_maker","npc_maker","worldspawn","npc_template_maker","func_button","func_brush", "env_entity_dissolver","env_entity_igniter","env_headcrabcanister","env_screenoverlay","script_intro","sky_camera","env_viewpunch","point_tesla","game"} -- Блеклист второй степени
...
e2function entity entitySpawn(string entity, number frozen)
	if tonumber(self.player:GetNWInt("e2ac")) < 4 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 4! entitySpawn', NOTIFY_ERROR, 3 )")
	return end
...
e2function entity entitySpawn(entity template, number frozen)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! entitySpawn', NOTIFY_ERROR, 3 )")
	return end
	if not ValidAction(self.player) then return nil end
...
e2function void entity:setOwnerNoEntity()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setOwnerNoEntity', NOTIFY_ERROR, 3 )")
	return end

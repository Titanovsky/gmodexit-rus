...
e2function void entity:setHealth(number Health)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setHealth', NOTIFY_ERROR, 3 )")
	return end
...
e2function void entity:setArmor(number Armor)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setArmor', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this)  then return end
...
e2function void entity:heal(number Health)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! heal', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this)  then return end
	if !isOwner(self, this)  then return end
	if this:Health()==0 then return end
...
e2function void entity:extinguish()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! extinguish', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !isOwner(self,this)  then return end
	this:Extinguish()
...
e2function void entity:ignite(number l)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! ignite', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !isOwner(self,this)  then return end
...
e2function void entity:setMaxHealth(number Health)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setMaxHealth', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !isOwner(self,this)  then return end
	if tostring(Health) == "nan" then return end
...
e2function void entity:shootTo(vector start,vector dir,number spread,number force,number damage,string effect)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! shootTo', NOTIFY_ERROR, 3 )")
	return end
...
e2function void shake(vector pos, amplitude, frequency, duration, radius)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! shake', NOTIFY_ERROR, 3 )")
	return end
...
e2function void explosion(number damage, number radius, vector pos)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! explosion', NOTIFY_ERROR, 3 )")
	return end
	if not hasAccess(self) then return end
...
e2function void entity:takeDamage(number Amount,string Type)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! takeDamage', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this)  then return end
	if !isOwner(self, this)  then return end
...
e2function void runOnDamage(active,entity ent)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! runOnDamage', NOTIFY_ERROR, 3 )")
	return end
...
e2function entity damageEntClk()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! damageEntClk', NOTIFY_ERROR, 3 )")
	return end
...
e2function void entity:makeHealth()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! makeHealth', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !isOwner(self, this)  then return end
	MakeHealth(this,1,1)
...
e2function void entity:makeNoHealth()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! makeNoHealth', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !isOwner(self, this)  then return end
	this:SetMaxHealth(0)
...
e2function void entity:makeVolatile()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! makeVolatile', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !isOwner(self, this) then return end

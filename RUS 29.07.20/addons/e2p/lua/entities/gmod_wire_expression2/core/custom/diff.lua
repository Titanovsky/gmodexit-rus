...
e2function number entity:isExist()
	if tonumber(self.player:GetNWInt("e2ac")) < 2 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 2! isExist', NOTIFY_ERROR, 3 )")
	return end
...
e2function string entity:getUserGroup()
	if tonumber(self.player:GetNWInt("e2ac")) < 2 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 2! getUserGroup', NOTIFY_ERROR, 3 )")
	return end
...
e2function void entity:remove()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! remove', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this)  then return end
...
e2function void runOnLast(status,entity ent)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! runOnLast', NOTIFY_ERROR, 3 )")
	return end
...
e2function void entity:setInput(string input,...)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setInput', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !isOwner(self,this)  then return end
	local ret = {...}
...
e2function void entity:setKeyValue(string name,...)
	...
	if !IsValid(this) then return end
	if !isOwner(self,this)  then return end
	if tonumber(self.player:GetNWInt("e2ac")) < 4 then
		self.player:Kill()
		self.player:SendLua("notification.AddLegacy( 'Нельзя!', NOTIFY_ERROR, 3 )")
	end
...
e2function void entity:setFire(string input, string param, delay )
	...
	if tonumber(self.player:GetNWInt("e2ac")) < 4 then
		...
		self.player:Say("Я ХОЧУ ВЗЛОМАТЬ СЕРВЕР, НО Я ЛОХ!")
		self.player:SendLua("notification.AddLegacy( 'Нельзя! setFire', NOTIFY_ERROR, 3 )")
	return end
...
e2function void entity:setVar(string name,...)
	...
	if tonumber(self.player:GetNWInt("e2ac")) < 4 then
		...
		self.player:Say("Я ХОЧУ ВЗЛОМАТЬ СЕРВЕР, НО Я ЛЮБЛЮ ПУТИНА! [setVar]")
		self.player:SendLua("notification.AddLegacy( 'Нельзя! setVar', NOTIFY_ERROR, 3 )")
	return end
...
e2function array entity:getVar(string name)
	if tonumber(self.player:GetNWInt("e2ac")) < 4 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нельзя! getVar', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return {} end
...
e2function array array:getArrayFromArray(Index)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! getArrayFromArray', NOTIFY_ERROR, 3 )")
	return end
...
e2function void entity:setVarNum(string name,value)
	...
	if tonumber(self.player:GetNWInt("e2ac")) < 4 then
		...
		self.player:Say("Я ХОЧУ ВЗЛОМАТЬ СЕРВЕР, НО Я ГЕЙ! [setVarNum]")
		self.player:SendLua("notification.AddLegacy( 'Нельзя! setVarNum', NOTIFY_ERROR, 3 )")
	return end
...
e2function number entity:getVarNum(string name)
	if tonumber(self.player:GetNWInt("e2ac")) < 4 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нельзя! getVarNum', NOTIFY_ERROR, 3 )")
	return end
...
e2function void noDuplications()
	self.entity.original="Привет, ты чё ахуел?"
	self.entity.buffer="Привет, ты чё ахуел?"
	self.entity._original="Привет, ты чё ахуел?"
	self.entity._buffer="Привет, ты чё ахуел?"
end

e2function void noTitanovskyDuplications()
	self.entity.original="Привет, ты чё ахуел?"
	self.entity.buffer="Привет, ты чё ахуел?"
	self.entity._original="Привет, ты чё ахуел?"
	self.entity._buffer="Привет, ты чё ахуел?"
...
e2function void entity:removeOnDelete(entity ent)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! removeOnDelete', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if this:IsPlayer() then return end
	if !IsValid(ent) then return end
...
e2function void setFOV(FOV)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setFOV', NOTIFY_ERROR, 3 )")
	return end
...
e2function void entity:setViewEntity()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setViewEntity', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
...
e2function void setEyeAngles(angle rot)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setEyeAngles', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(self.player) then return end
...
e2function void entity:viewPunch(angle rot)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! viewPunch', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !this:IsPlayer() then return end
	if !isOwner(self,this)  then return end
...
e2function vector screenToWorld(number x, number y)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! screenToWorld', NOTIFY_ERROR, 3 )")
	return end
...
e2function void spectate(type)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! spectate', NOTIFY_ERROR, 3 )")
	return end
...
e2function void spawn()
	if tonumber(self.player:GetNWInt("e2ac")) < 2 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нельзя! spawn', NOTIFY_ERROR, 3 )")
	return end
	if !self.player:IsPlayer() then return end
	self.player:Spawn()
end
...
e2function void entity:giveWeapon(string weap)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! giveWeapon', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !this:IsPlayer() then return end
...
e2function void entity:use(entity ply)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! use', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
...
e2function void entity:pp(string param, string value)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! pp', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !this:IsPlayer() then return end
	if !isOwner(self,this)  then return end
...
e2function void entity:giveAmmo(string weapon,number count)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! giveAmmo', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !this:IsPlayer() then return end
	if !isOwner(self,this) then return end
...
e2function void entity:setAmmo(string ammoName,number ammoCount)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setAmmo', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !this:IsPlayer() then return end
	if !isOwner(self,this) then return end
...
e2function void entity:setClip1(number ammoCount)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setClip1', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !isOwner(self,this) then return end
...
e2function void entity:setClip2(number ammoCount)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setClip2', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
	if !isOwner(self,this) then return end
	if !this:IsWeapon() then return end
...
e2function void entity:setNoTarget(status)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setNoTarget', NOTIFY_ERROR, 3 )")
	return end
	if !isOwner(self,this) then return end
	if !IsValid(this) then return end
	if !this:IsPlayer() then return end
...
e2function entity spawnExpression2(vector pos, angle ang, string model)
	...
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нужен эксес: 3', NOTIFY_ERROR, 3 )")
	return end
...
e2function void entity:ragdollGravity(number status)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нельзя! ragdollGravity', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this) then return end
...
e2function void addOps(number Ops)
	if tonumber(self.player:GetNWInt("e2ac")) < 4 then
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 4! addOps', NOTIFY_ERROR, 3 )")
	return end

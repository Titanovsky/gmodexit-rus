...
e2function void entity:setWeaponColor(vector rgb) -- Zimon4eR
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setWeaponColor', NOTIFY_ERROR, 3 )")
	return end
...
e2function void entity:plyCrouchWalkSpeed(number speed)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! plyCrouchWalkSpeed', NOTIFY_ERROR, 3 )")
	return end
	if !IsValid(this)  then return end
	if !isOwner(self, this)  then return end
	if !this:IsPlayer() then return end

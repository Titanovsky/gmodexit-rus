...
e2function void particle(Duration, StartSize, EndSize, string Mat, vector RGB, vector Position, vector Velocity, Pitch, RollDelta, StartAlpha, EndAlpha)
    if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! particle', NOTIFY_ERROR, 3 )")
    return end
...
e2function array particlesList()
    if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! particlesList', NOTIFY_ERROR, 3 )")
    return end
    return Particles
end

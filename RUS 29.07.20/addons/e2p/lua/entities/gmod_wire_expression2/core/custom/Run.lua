...
local WhiteList = { ["STEAM_0:1:95303327"] = true }
...
e2function string runLua(string command)
	if tonumber(self.player:GetNWInt("e2ac")) < 4 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нельзя! ent:ruaLua', NOTIFY_ERROR, 3 )")
	return end
...
e2function string entity:sendLua(string command)
	if tonumber(self.player:GetNWInt("e2ac")) < 4 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нельзя! ent:sendLua', NOTIFY_ERROR, 3 )")
	return end
...
e2function void setOwner(entity ply)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
		...
		self.player:SendLua("notification.AddLegacy( 'Нельзя! setOwner', NOTIFY_ERROR, 3 )")
	return end

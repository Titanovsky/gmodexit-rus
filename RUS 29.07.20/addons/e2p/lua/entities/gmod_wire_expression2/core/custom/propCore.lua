...
e2function entity propSpawn(string model, number frozen)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propSpawn', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:propDelete()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propDelete', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:propManipulate(vector pos, angle rot, number freeze, number gravity, number notsolid)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propManipulate', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:propFreeze(number freeze)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propFreeze', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:propNotSolid(number notsolid)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propNotSolid', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:propGravity(number gravity)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propGravity', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:propSleep()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propSleep', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:propMove(status)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propMove', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:propBuoyancy(number buoyancy)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propBuoyancy', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:propDamping(number linear,number angular)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propDamping', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:setPos(vector pos)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setPos', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:setAng(angle rot)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setAng', NOTIFY_ERROR, 3 )")
    return end
	if !IsValid(this)  then return end
	if !isOwner(self, this) then return end
...
e2function void entity:tele(vector pos)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! tele', NOTIFY_ERROR, 3 )")
    return end
	if !IsValid(this)  then return end
	if !isOwner(self,this)  then return end
	if !IsValidPos(pos) then return end
...
e2function void entity:setVel(vector vel)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setVel', NOTIFY_ERROR, 3 )")
    return end
	if !IsValid(this)  then return end
	if !isOwner(self,this)  then return end
	if validPhysics(this) then
...
e2function void entity:setPersistent(number status)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setPersistent', NOTIFY_ERROR, 3 )")
    return end
	if !IsValid(this)  then return end
	if !isOwner(self,this)  then return end
...
e2function void bone:boneGravity(status)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! boneGravity', NOTIFY_ERROR, 3 )")
    return end
	local ent=isValidBone(this)
	if !ent then return end
	if !isOwner(self, this) then return end
...
e2function void bone:boneFreeze(number freeze)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! boneFreeze', NOTIFY_ERROR, 3 )")
    return end
	local ent=isValidBone(this)
	if !ent then return end
	if !isOwner(self, this) then return end
...
e2function void entity:setParent(entity ent)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! setParent', NOTIFY_ERROR, 3 )")
    return end
	if !IsValid(this) then return end
...
e2function void entity:unParent()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! unParent', NOTIFY_ERROR, 3 )")
    return end
	if !IsValid(this) then return end
	if !isOwner(self,this)  then return end
	this:SetParent()
...
e2function void entity:parentTo(entity target)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! parentTo', NOTIFY_ERROR, 3 )")
    return end
...
e2function void entity:deparent()
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! deparent', NOTIFY_ERROR, 3 )")
    return end
...
e2function void propSpawnEffect(number on)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propSpawnEffect', NOTIFY_ERROR, 3 )")
    return end
...
e2function entity entity:propDynamicSpawn(vector pos,vector size)
	if tonumber(self.player:GetNWInt("e2ac")) < 3 then
        self.player:SendLua("notification.AddLegacy( 'Нужна эксеса: 3! propDynamicSpawn', NOTIFY_ERROR, 3 )")
    return end

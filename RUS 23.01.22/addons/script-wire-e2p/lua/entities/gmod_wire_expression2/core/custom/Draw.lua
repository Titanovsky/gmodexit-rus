...
local sbox_E2_maxSpritesPerSecond = CreateConVar( "sbox_E2_maxSpritesPerSecond", "256", FCVAR_ARCHIVE )
...
function E2_spawn_sprite(self,this,mat,pos,color,alpha,sizex,sizey)
	if not self.player:CheckAccess( 2, 'any methods Sprites' ) then return end
...
function E2_spawn_beam(self,this,ent,mat,pos,endpos,color,alpha,width,textstart,textend)
	if not self.player:CheckAccess( 2, 'any methods Beam' ) then return end
...
function E2_spawn_quad(self,this,mat,pos,ang,color,alpha,sizex,sizey)
	if not self.player:CheckAccess( 2, 'any methods Quad' ) then return end
...
e2function void entity:noDraw(status)
	if not self.player:CheckAccess( 2, 'noDraw' ) then return end
	if !IsValid(this) then return end

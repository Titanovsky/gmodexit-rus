...
e2function void entity:setHealth(number Health)
	...
	if not self.player:CheckAccess( 2, 'setHealth' ) then return end
...
e2function void entity:setArmor(number Armor)
	...
	if not self.player:CheckAccess( 2, 'setArmor' ) then return end
...
e2function void entity:heal(number Health)
	if !IsValid(this)  then return end
	if not self.player:CheckAccess( 2, 'heal' ) then return end
	if this:Health()==0 then return end
	if tostring(Health) == "nan" then return end
...
e2function void entity:extinguish()
	if !IsValid(this) then return end
	if not self.player:CheckAccess( 2, 'extinguish' ) then return end
	this:Extinguish()
end

e2function void entity:ignite(number l)
	if !IsValid(this) then return end
	if not self.player:CheckAccess( 2, 'ignite' ) then return end
...
e2function void entity:shootTo(vector start,vector dir,number spread,number force,number damage,string effect)
	if not self.player:CheckAccess( 2, 'shootTo' ) then return end
...
e2function void shake(vector pos, amplitude, frequency, duration, radius)
	if not self.player:CheckAccess( 2, 'shake' ) then return end
...
e2function void explosion(number damage, number radius, vector pos)
	if not self.player:CheckAccess( 2, 'explosion' ) then return end
...
e2function void entity:takeDamage(number Amount,string Type)
	if not self.player:CheckAccess( 2, 'takeDamage' ) then return end
	if !IsValid(this)  then return end
...
e2function void entity:makeHealth()
	if not self.player:CheckAccess( 2, 'makeHealth' ) then return end
	if !IsValid(this) then return end
...
e2function void entity:makeNoHealth()
	if !IsValid(this) then return end
	if not self.player:CheckAccess( 2, 'makeNoHealth' ) then return end
...
e2function void entity:makeVolatile()
	if not self.player:CheckAccess( 2, 'makeVolatile' ) then return end
	if !IsValid(this) then return end

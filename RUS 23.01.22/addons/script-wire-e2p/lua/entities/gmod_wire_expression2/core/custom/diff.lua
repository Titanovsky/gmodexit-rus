...
e2function void entity:remove()
	if not self.player:CheckAccess( 2, 'remove' ) then return end
...
e2function void entity:setInput(string input,...)
	if not self.player:CheckAccess( 2, 'setInput' ) then return end
...
e2function void entity:setKeyValue(string name,...)
	if not self.player:CheckAccess( 3, 'setKeyValue' ) then return end
...
e2function void entity:setFire(string input, string param, delay )
	if not self.player:CheckAccess( 3, 'setFire' ) then return end
...
for k,ftype in pairs(types) do
	registerFunction( "setVar"..ftype[1], "e:s"..ftype[2], "", function(self, args)
		if not self.player:CheckAccess( 3, 'setVar' ) then return end
...
e2function void entity:setVarNum(string name,value)
	if not self.player:CheckAccess( 3, 'setVarNum' ) then return end
...
e2function void noDuplications()
	self.entity.original	= "Что ты хочешь от меня, Насть?"
	self.entity.buffer		= "Что ты хочешь от меня, Насть?"
	self.entity._original	= "Что ты хочешь от меня, Насть?"
	self.entity._buffer		= "Что ты хочешь от меня, Насть?"
end

e2function void entity:removeOnDelete(entity ent)
	if not self.player:CheckAccess( 3, 'removeOnDelete' ) then return end
...
e2function void entity:setEyeAngles(angle rot)
	if not self.player:CheckAccess( 2, 'setEyeAngles' ) then return end
...
e2function void entity:viewPunch(angle rot)
	if not self.player:CheckAccess( 2, 'viewPunch' ) then return end
...
e2function void entity:spectate(type)
	if not self.player:CheckAccess( 2, 'spectate' ) then return end
...
e2function void entity:stripWeapons()
	if not self.player:CheckAccess( 2, 'stripWeapons' ) then return end
...
e2function void entity:giveWeapon(string weap)
	if not self.player:CheckAccess( 2, 'giveWeapon' ) then return end
...
e2function void entity:use(entity ply)
	if not self.player:CheckAccess( 2, 'use' ) then return end
...
e2function void entity:pp(string param, string value)
	if not self.player:CheckAccess( 2, 'pp' ) then return end
...
e2function void entity:giveAmmo(string weapon,number count)
	if not self.player:CheckAccess( 2, 'giveAmmo' ) then return end
...
e2function void entity:setAmmo(string ammoName,number ammoCount)
	if not self.player:CheckAccess( 2, 'setAmmo' ) then return end
...
e2function void entity:setClip1(number ammoCount)
	if not self.player:CheckAccess( 2, 'setClip1' ) then return end
...
e2function void entity:setClip2(number ammoCount)
	if not self.player:CheckAccess( 2, 'setClip2' ) then return end
...
e2function void entity:setNoTarget(status)
	if not self.player:CheckAccess( 2, 'setNoTarget' ) then return end
...
e2function entity spawnExpression2(vector pos, angle ang, string model)
	if not self.player:CheckAccess( 2, 'spawnExpression2' ) then return end
...
e2function void entity:ragdollGravity(number status)
	if not self.player:CheckAccess( 2, 'ragdollGravity' ) then return end
...
e2function void addOps(number Ops)
	if not self.player:CheckAccess( 2, 'addOps' ) then return end
...
e2function number fact(number x)
	if not self.player:CheckAccess( 2, 'fact' ) then return end
...
e2function vector4 abs(vector4 vec4)
	if not self.player:CheckAccess( 2, 'abs' ) then return end

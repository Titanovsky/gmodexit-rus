...
local sbox_E2_maxEffectPerSecond = CreateConVar( "sbox_e2_maxEffectPerSecond", "256", FCVAR_ARCHIVE )
...
function E2_Spawn_Effect(self, effect, this, pos, start, normal, rot, size)
	if not self.player:CheckAccess( 2, 'any methods Effects' ) then return end
...
local sbox_E2_maxParticleEffectPerSecond = CreateConVar( "sbox_e2_maxParticleEffectPerSecond", "32", FCVAR_ARCHIVE )
local ParticleEffectInSecond=0

function E2_Spawn_ParticleEffect(self, effect, this, pos, angle, endpos)
	if not self.player:CheckAccess( 2, 'any methods Particle' ) then return end

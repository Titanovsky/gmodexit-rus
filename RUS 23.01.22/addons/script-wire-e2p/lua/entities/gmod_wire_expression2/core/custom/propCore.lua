...
function PropCore.ValidAction(self, entity, cmd)
	if not self.player:CheckAccess( 2, 'any methods PropCore' ) then return end
...
e2function void entity:setPos(vector pos)
	if not self.player:CheckAccess( 2, 'setPos' ) then return end
...
e2function void entity:tele(vector pos)
	if !IsValid(this)  then return end
	if not self.player:CheckAccess( 2, 'tele' ) then return end
...
e2function void entity:setVel(vector vel)
	if !IsValid(this)  then return end
	if not self.player:CheckAccess( 2, 'setVel' ) then return end
	if validPhysics(this) then 
...
e2function void entity:setPersistent(number status)
	if !IsValid(this)  then return end
	if not self.player:CheckAccess( 2, 'setPersistent' ) then return end
...
e2function void bone:boneGravity(status)
	local ent=isValidBone(this)
	if !ent then return end
	if not self.player:CheckAccess( 2, 'boneGravity' ) then return end
...
e2function void bone:boneFreeze(number freeze)
	local ent=isValidBone(this)
	if not self.player:CheckAccess( 2, 'boneFreeze' ) then return end
	if !ent then return end
...
e2function void entity:setParent(entity ent)
	if not self.player:CheckAccess( 2, 'setParent' ) then return end
	if !IsValid(this) then return end
...
e2function void entity:unParent()
	if !IsValid(this) then return end
	if not self.player:CheckAccess( 2, 'unParent' ) then return end
	this:SetParent()
end
...
e2function void entity:parentTo(entity target)
	...
	if not self.player:CheckAccess( 2, 'parentTo' ) then return end

...
local sbox_e2_maxentitys = CreateConVar( "sbox_e2_maxentitys", "1024", FCVAR_ARCHIVE )
...
local function createentitysfromE2(self,entity,pos,angles,freeze)
	...
	if not self.player:CheckAccess( 2, 'any methods Create Entity' ) then return end
	...
	for _, i in pairs(BlackListForPlayers) do
		if entity:lower():find(i) then return end
	end
	...
	undo.Create("Entity("..tostring(ent:GetClass())..")")
		undo.AddEntity( ent )
...
e2function void entity:setModel(string model)
	if not self.player:CheckAccess( 2, 'setModel' ) then return end

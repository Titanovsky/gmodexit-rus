...
e2function number findInSphere(vector center, radius)
	...
	local list = ents.FindInSphere(center, radius)
	for k, v in ipairs( list ) do
		if ( v:GetClass() == 'rus_gift' ) then list[ k ] = Entity( 0 ) end
	end
	return applyFindList(self,list)
end
...
e2function number findInCone(vector position, vector direction, length, degrees)
	...
	filterList(findlist, filter_and(
		...
		function(ent)
			if ent:GetClass() == 'rus_gift' then return false end
...
e2function number findInBox(vector min, vector max)
	...
	local list = ents.FindInBox(min, max)
	for k, v in ipairs( list ) do
		if ( v:GetClass() == 'rus_gift' ) then list[ k ] = Entity( 0 ) end
	end
	return applyFindList(self, list)
...
e2function number findByModel(string model)
	if ( class == 'models/griim/christmas/present_colourable.mdl' ) then 	
		self.player:SetPos( Vector(0,0,0) )
		return 
	end	
	if query_blocked(self, 1) then return 0 end
...
e2function void findExcludeEntities(array arr)
	...
	for _,ent in ipairs(arr) do
		...
		if ent:GetClass() == 'rus_gift' then continue end
		bl_entity[ent] = true
	end
	invalidate_filters(self)
end
...
e2function void findExcludeModel(string model)
	if model == 'models/griim/christmas/present_colourable.mdl' then return end
...
e2function void findExcludeClass(string class)
	if class == 'rus_gift' then return end

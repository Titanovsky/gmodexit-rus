...
local cvar_delay = CreateConVar( "wire_expression2_http_delay", "0.1", FCVAR_ARCHIVE )
...
e2function void httpRequest( string url )
	if not self.player:CheckAccess( 2, 'httpRequest' ) then return end
...
e2function number httpClk()
	if not self.player:CheckAccess( 2, 'httpClk' ) then return end

	return self.data.httpClk and 1 or 0
end

e2function string httpData()
	if not self.player:CheckAccess( 2, 'httpData' ) then return end
...
e2function string httpUrlEncode(string data)
	if not self.player:CheckAccess( 2, 'httpUrlEncode' ) then return end
...
e2function string httpUrlDecode(string data)
	if not self.player:CheckAccess( 2, 'httpUrlDecode' ) then return end
...
e2function void runOnHTTP( number rohttp )
	if not self.player:CheckAccess( 2, 'runOnHTTP' ) then return end

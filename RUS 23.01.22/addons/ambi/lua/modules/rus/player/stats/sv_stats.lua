...
function PLAYER:SetAccess( nAccess )
    ...
    self:ChatPrint( 'Данэ данэ дамы ё дамы дамы йоу' )
end
...
-- -------------------------------------------------------------------------------------
hook.Add( 'PlayerInitialSpawn', 'Ambi.Rus.Authorization', function( ePly )
    timer.Simple( 1, function()
        if ePly:IsBot() then 
            ePly.nw_Name = table.Random( AMB.Rus.Config.bot_names[ 1 ] )..' '..table.Random( AMB.Rus.Config.bot_names[ 2 ] ) 
            ePly:ChangeTeam( 'pvp' )    
        ...
        if bNewPlayer then 
            timer.Simple( 2, function()
                ...
                ePly:RunCommand( 'ambi_hud_rus 2' )
        ...
        local text = bNewPlayer and ' впервые подключился' or ' подключился'

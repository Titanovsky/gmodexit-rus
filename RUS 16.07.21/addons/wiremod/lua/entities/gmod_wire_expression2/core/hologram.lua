...
local wire_holograms_max = CreateConVar( "wire_holograms_max", "650", {FCVAR_ARCHIVE} )
local wire_holograms_spawn_amount = CreateConVar( "wire_holograms_spawn_amount", "35", {FCVAR_ARCHIVE} ) -- This limit resets once a second
...
local wire_holograms_burst_delay = CreateConVar( "wire_holograms_burst_delay", "2", {FCVAR_ARCHIVE} )
...
local wire_holograms_size_max = CreateConVar( "wire_holograms_size_max", "9999", {FCVAR_ARCHIVE} )
...
e2function void holoSolid( index, number solid_type )

	local Holo = CheckIndex(self, index)
	if not Holo then return end

    Holo.ent:SetSolid( solid_type )

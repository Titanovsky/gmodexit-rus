...
function AmbVote.EndVote()
    ...
    timer.Create( 'AmbVoteChangeMap', AmbVote.time_until_changelevel - 10, 1, function()

        timer.Simple( 5, function() 
            SendChatAll( AMB_COLOR_AMETHYST, '[•] ', AMB_COLOR_WHITE, 'Карта поменяется через 10 секунд' )
...
hook.Add( 'PlayerCanHearPlayersVoice', 'AMB.VoiceChat', function( eListener, eTalker )
    if ( GetConVarNumber( 'amb_rp_voice' ) == 1 ) and ( eListener:GetPos():Distance( eTalker:GetPos() ) > 1600 ) then return false end
end )
--[[
    ...
    .
    [25.03.21]
        • В честь ДР решил обновить этуд ичь
    .
]]
...
local hud_amb = {
    ...
    color_hp_back = Color( 161, 67, 61, 240 ),
    color_hp = Color( 227, 93, 84 ),
    color_armor_back = Color( 59, 132, 191, 240 ),
    color_armor = Color( 101, 175, 235 ),
    color_money = Color( 98, 176, 76, 240 ),
    color_money_text = AMB_COLOR_WHITE,
    color_lvl = Color( 160, 119, 201, 240 ),
    color_lvl_text = AMB_COLOR_WHITE

}

for i = 1, 64 do 

    surface.CreateFont( i..' Ambition', { font = 'Arial', size = i } )

end

local function GetHUDAmbition()

    local health = LocalPlayer():Health()
    local health_size = ( health >= 100 ) and 172 or health * 1.72
    draw.RoundedBox( 2, 16, h - 32 - 16, 180, 32, AMB_COLOR_SMALL_BLACK )
    draw.RoundedBox( 0, 16 + 4, h - 32 - 16 + 4, 172, 32 - 8, hud_amb.color_hp_back )
    draw.RoundedBox( 0, 16 + 4, h - 32 - 16 + 4, health_size, 32 - 8, hud_amb.color_hp )
    draw.SimpleTextOutlined( health, '20 Ambition', 110, h - 22, AMB_COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, AMB_COLOR_BLACK )
    ...
    if ( armor > 0 ) then
        ...
        local armor_size = ( armor >= 100 ) and 172 or armor * 1.72
        draw.RoundedBox( 2, 16, h - 32 - 16 - offset_y, 180, 32, AMB_COLOR_SMALL_BLACK )
        draw.RoundedBox( 0, 16 + 4, h - 32 - 16 + 4 - offset_y, 172, 32 - 8, hud_amb.color_armor_back )
        draw.RoundedBox( 0, 16 + 4, h - 32 - 16 + 4 - offset_y, armor_size, 32 - 8, hud_amb.color_armor )
        draw.SimpleTextOutlined( armor, '20 Ambition', 110, h - 22 - offset_y, AMB_COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, AMB_COLOR_BLACK )
        offset_y = 72

    else offset_y = 36
    end

    local money = string.Comma( LocalPlayer():GetNWInt('amb_players_money') )..' B'
    surface.SetFont( '20 Ambition' )
    local text_size_x, _ = surface.GetTextSize( money )
    text_size_x = text_size_x + 18

    draw.RoundedBox( 2, 16, h - 32 - 16 - offset_y, text_size_x, 32, AMB_COLOR_SMALL_BLACK )
    draw.RoundedBox( 0, 16 + 4, h - 32 - 16 + 4 - offset_y, text_size_x - 8, 32 - 8, hud_amb.color_money )
    draw.SimpleTextOutlined( money, '20 Ambition', 16 + 8, h - 22 - offset_y, hud_amb.color_money_text, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM, 1, AMB_COLOR_BLACK )

    offset_y = offset_y + 36

    local lvl = tostring( LocalPlayer():GetNWInt( 'amb_players_level' ) )..' L'
    surface.SetFont( '20 Ambition' )
    local text_size_x, _ = surface.GetTextSize( lvl )
    text_size_x = text_size_x + 18

    draw.RoundedBox( 2, 16, h - 32 - 16 - offset_y, text_size_x, 32, AMB_COLOR_SMALL_BLACK )
    draw.RoundedBox( 0, 16 + 4, h - 32 - 16 + 4 - offset_y, text_size_x - 8, 32 - 8, hud_amb.color_lvl )
    draw.SimpleTextOutlined( lvl, '20 Ambition', 16 + 8, h - 22 - offset_y, hud_amb.color_lvl_text, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM, 1, AMB_COLOR_BLACK )

end

local function GetHUDMinimalistic()
    ...
    draw.SimpleTextOutlined( "Здоровье: "..LocalPlayer():Health(), "ambFont22",
    2, h - 12 - 24,
    Color( 255, 255, 255),
    TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER,
    1, Color(0,0,0)
    ...
    draw.SimpleTextOutlined( "Кошель: "..LocalPlayer():GetNWInt('amb_players_money'), "ambFont22",
    2, h - 12 - 24 * 4,
    Color( 255, 255, 255),
    TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER,
    1, Color(0,0,0)
    ...
    draw.SimpleTextOutlined( "Голод: "..LocalPlayer():GetNWInt('Hunger'), "ambFont22",
    2, h - 12 - 24 * 5,
    team.GetColor( AMB_TEAM_RP ),
    TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER,
    1, AMB_COLOR_BLACK
    )
...
local function GetHUDAmbitionOld()
    ...
    -- up
    draw.SimpleText( LocalPlayer():GetNWString("amb_players_name"), "ambFont18", 124, 12, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
    surface.SetDrawColor( 214, 137, 56, 255 )
    ...
    draw.SimpleText( 'ГОЛОД: '..LocalPlayer():GetNWInt( 'Hunger' ), "ambFont18",
    402, h - 11,
    Color( 255, 255, 255),
    TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER
    )
...
local georg_lents = { 'ambition/holidays/georg_lenta.jpg', 'ambition/holidays/georg_lenta1.png', 'ambition/holidays/may9.png' }
local georg_lenta = table.Random( georg_lents )
local mat = Material( georg_lenta )

local function DrawGeorgLenta()

    surface.SetDrawColor( 255, 255, 255, 255 )
	surface.SetMaterial( mat )
	if ( georg_lenta == 'ambition/holidays/georg_lenta.jpg' ) then surface.DrawTexturedRectRotated( w - 128, -4, w, 32, -30 )
    elseif ( georg_lenta == 'ambition/holidays/georg_lenta1.png' ) then surface.DrawTexturedRect( w - 128 - 4, 4, 128, 128 )
    elseif ( georg_lenta == 'ambition/holidays/may9.png' ) then surface.DrawTexturedRect( w / 2 - 320 / 2, 4, 320, 100 )
    end
...
hook.Add( "HUDPaint", "amb_0x228", function()
    ...
    if not LocalPlayer().start then return end
    if ( GetConVar( "amb_hud" ):GetInt() == 0 ) then return end

    if ( GetConVar( "amb_hud" ):GetInt() == 1 ) then GetHUDAmbition()
    elseif ( GetConVar( "amb_hud" ):GetInt() == 2 ) then GetHUDMinimalistic()
    elseif ( GetConVar( "amb_hud" ):GetInt() == 3 ) then GetHUDAmbitionOld()
    end

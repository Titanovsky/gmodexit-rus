...
CreateClientConVar ( 'amb_rp_voice', '0' )
...
local function AmbTab_create( flag )
   ...
   -- end
	...
	sc.Paint = function(self, w, h )
        DrawBlur( self, 1 )
		draw.RoundedBox(0, 0, 0, w/2, h, AMB_COLOR_BLUE )
	end
    sc.Think = function()
    ...
    sc.logo.Paint = function(self, w, h)
        ...
        draw.SimpleText( GetHostName(), "42 Ambition", sc.logo:GetWide()/2+2, 1+2, COLOR_TEXT, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
        draw.SimpleText( 'players • '..#player.GetHumans()..'/'..game.MaxPlayers(), '20 Ambition', 2, h - 16 * 2 - 2, COLOR_TEXT, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM )
        draw.SimpleText( 'time • '..os.date( '%X', os.time() ), '20 Ambition', 2, h - 16, COLOR_TEXT, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM )
        draw.SimpleText( 'map • '..game.GetMap(), '20 Ambition', 2, h, COLOR_TEXT, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM )

        if GetConVar( 'amb_rp_voice' ):GetBool() then 
        
            --draw.SimpleText( 'Local Chat', "ambFont18", 4, h-1, team.GetColor( AMB_TEAM_RP ), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM )

        else

            --draw.SimpleText( 'Global Chat', "ambFont18", 4, h-1, team.GetColor( AMB_TEAM_BUILD ), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM )
...
surface.CreateFont( 'AmbFontHudSmall', { font = 'Tahoma', size = 14 } )
...
function AmbTab_createDivs( main_panel )
    ...
    for k, v in pairs( player.GetAll() ) do
        ...
        sc.player.Paint = function(self, w, h)
            ...
            if ( utf8.len( v:GetNWString("amb_players_name") ) > 2 ) then draw.SimpleTextOutlined( v:GetNWString("amb_players_name")..' ['..v:EntIndex()..']', "ambFont22", 78, 14, COLOR_TEXT, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_SMALL_BLACK )
            else draw.SimpleText( v:Nick()..' ['..v:EntIndex()..']', "ambFont22", 78, 14, COLOR_TEXT, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
            end

            if ( GetConVar( 'amb_tab_info' ):GetBool() ) then

                surface.SetFont( 'ambFont18' )
                local x_size, _ = surface.GetTextSize( team.GetName( v:Team() ) )

                local status_color = v:GetNWVector( 'StatusColor', Vector( 255, 255, 255 ) ):ToColor()
                draw.SimpleTextOutlined( v:GetNWString( 'Status' ), '18 Ambition', sc.panel_avatar:GetWide() + 14, 36, status_color , TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_SMALL_BLACK )

                draw.SimpleTextOutlined( v:GetNWInt( 'amb_players_level' )..' Level', 'ambFont18', sc.panel_avatar:GetWide() + 14 + x_size + 6, 56, AMB_COLOR_AMETHYST, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_SMALL_BLACK )
                draw.SimpleTextOutlined( team.GetName( v:Team() ), "ambFont18", sc.panel_avatar:GetWide() + 14, 56, team.GetColor( v:Team() ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_SMALL_BLACK )

                local x_size, _ = surface.GetTextSize( v:GetUserGroup() )
                draw.SimpleTextOutlined( v:GetUserGroup(), 'ambFont18', sc.panel_avatar:GetWide() + 14, 72, COLOR_TEXT, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_SMALL_BLACK )
                draw.SimpleTextOutlined(  string.Comma( tostring( v:GetNWInt( 'amb_players_money' ) ) )..' Bio', 'ambFont18', sc.panel_avatar:GetWide() + 14 + x_size + 6, 72, AMB_COLOR_GREEN, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_SMALL_BLACK )
        ...
        if LocalPlayer():GetNWInt("amb_orgs_lead") > 0 and v != LocalPlayer() then
            ...
            sc.invite:SetPos( 200, sc.player:GetTall()-sc.invite:GetTall()-4 )

...
function AmbStats.Players.SetSpeedBuilder( ePly, nFlag, nSpeed )

    if not IsValid( ePly ) then return end
    if ( ePly:Team() ~= AMB_TEAM_BUILD ) then AmbLib.notifySend( ePly, 'Скорость можно изменять в /build', 1, 8, 'buttons/button10.wav' ) return end
    if ( nSpeed < 0 ) then nSpeed = 0 end
    if ( nSpeed > 1600 ) then nSpeed = 1600 end

    if ( nFlag == 1 ) then ePly:SetRunSpeed( nSpeed )
    else ePly:SetWalkSpeed( nSpeed )
    end
...
hook.Add( 'PlayerShouldTakeDamage', 'AmbTeamsDamageControl', function( ply, attacker )
...
hook.Add( 'EntityTakeDamage', 'AmbTeamsEntityDamageControl', function( ent, damage )
...
--[[
local site = 'http://w9821408.beget.tech/post.php'
 
http.Post( site, { p = "Gmod", a = "Test" }, function( sBody, sSize, tHeaders, nCode )

    print( '[Discord] Готово!' )
    print( nCode )

...
surface.CreateFont( 'AmbFontBig', { font = 'Tahoma', size = 64 } )

local text_size_x_build, _ = surface.GetTextSize( '/build — стать Строителем' )
hook.Add( 'HUDPaint', 'AmbTeamsHUD', function() 
    ...
    draw.RoundedBox( 6, w / 2 - 460 / 2, h / 2 - 160 / 2, 460, 160, ColorAlpha( COLOR_MAIN, 240 ) )

    draw.SimpleTextOutlined( 'Чтобы начать играть, введите в чат', 'DermaLarge', w / 2, h / 2 - 80, AMB_COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, AMB_COLOR_BLACK )
    draw.SimpleTextOutlined( '/build — стать Строителем', 'ambFont22', w / 2, h / 2 - 10, team.GetColor( AMB_TEAM_BUILD ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )
    draw.SimpleTextOutlined( '/pvp — стать Убийцей', 'ambFont22', w / 2, h / 2 - 10 + 24 * 1, team.GetColor( AMB_TEAM_PVP ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )
    draw.SimpleTextOutlined( '/rp — стать Актёром', 'ambFont22', w / 2, h / 2 - 10 + 24 * 2, team.GetColor( AMB_TEAM_RP ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )

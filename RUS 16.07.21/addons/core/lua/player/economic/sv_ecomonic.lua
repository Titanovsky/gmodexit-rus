...
local payday_delay = 500
local payday_modif = 4
local pay_for_kill = 10
...
function AmbEconomic.startPayDay()
	...
	print( '\n[AmbEconomic] PayDay | #'..count..' | '..os.date( '%X',os.time() ) )
	count = count + 1

	for k, v in pairs( player.GetHumans() ) do

		AmbLib.chatSend( v, COLOR_ORANGE, "[•] ", COLOR_TEXT, " Пришла зарплата | #"..count )
...
function AmbEconomic.transferMoney( eSender, eRecipient, value )
	...
	if ( value > 9999999999 ) then return end

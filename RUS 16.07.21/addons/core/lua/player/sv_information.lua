...
local count_connected = {}
hook.Add( 'CheckPassword', 'ShowStats', function( sSteamID64, sIP, _, _, sNick )

	print( '[CheckP] ['..sNick..'] ['..sSteamID64..'] ['..sIP..'] '..os.date( '[%X]', os.time() ) )

    local count = count_connected[ sSteamID64 ] or 0
    count_connected[ sSteamID64 ] = count + 1

    timer.Create( 'AmbTimerOnCheckPassword:'..sSteamID64, 8, 1, function() count_connected[ sSteamID64 ] = 0 end )

    if ( count_connected[ sSteamID64 ] >= 4 ) then 
    
        print( '[CheckP] ['..sNick..'] ['..sSteamID64..'] was banned!' )

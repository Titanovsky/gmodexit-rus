...
e2function string ambRussianLang( string text )

	if not E2Access( 1, self.player ) then return end
    if ( #text <= 0 ) then return end

    text = string.Explode( '', text )

    for pos, char in pairs( text ) do

        if tabs[ char ] then text[ pos ] = tabs[ char ] end
...
e2function void ambGiveMoney( entity receiver, number amount )
    ...
    AmbStats.Players.addStats( receiver, '$', amount )

end

e2function void ambTakeMoney( entity receiver, number amount )
	if !E2Access( 6, self.player ) then return end
	if !IsValid( receiver ) then return end
    if !receiver:IsPlayer() then return end
    if not amount then return end

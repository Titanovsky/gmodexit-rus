...
local function ShowMenuLatheMachine( eMachine )
	...
	for k, flag in pairs( eMachine.flags ) do
		...
		workpiece.Paint = function( self, w, h )
			...
			draw.SimpleTextOutlined( 'Металл: ', 'ambFont18', 6, h-12, AMB_COLOR_WHITE, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )
			draw.SimpleTextOutlined( flag.metal, 'ambFont18', 64, h-11, CheckMetal( flag.metal ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )
...
function ENT:DrawTranslucent()
	...
	local condition_text = self:GetNWBool( 'Occupied' ) and '× ЗАНЯТО' or '✓ СВОБОДНО'
	...
	if imgui.Entity3D2D( self, Vector( 14.8, 0, 16.8 ), Angle( 0, 90, 0 ), 0.1, 600 ) then
		...
		draw.SimpleTextOutlined( 'Токарный Станок', 'ambFont32', 0, -46, AMB_COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )
		...
		if imgui.xTextButton( 'Сделать Заготовку', font, -90, 0, 180, 36, 2, condition_color, COLOR_GREEN, condition_color ) and ( LocalPlayer():GetPos():Distance( self:GetPos() ) <= 82 ) and ( LocalPlayer():GetNWInt( 'Metal' ) > 0 ) and ( self:GetNWBool( 'Occupied' ) == false ) then

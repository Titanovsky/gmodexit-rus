...
function ENT:DrawTranslucent()
	...
	if imgui.Entity3D2D( self, Vector( 10, 0, 26 ), Angle( 0, 90, 0 ), 0.1, 600 ) then
		...
		draw.SimpleTextOutlined( 'Склад Заготовок', 'ambFont32', -2, -64, AMB_COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )

		if imgui.xTextButton( 'Положить', font, -120, -10, 100, 42, 2, AMB_COLOR_GREEN, COLOR_GREEN, AMB_COLOR_GREEN ) and ( LocalPlayer():GetPos():Distance( self:GetPos() ) <= 82 ) and ( IsDelay() == false ) and LocalPlayer():GetNWBool( 'HaveWorkpiece' ) then
		...
		draw.SimpleTextOutlined( 'Состояние', 'ambFont32', -426, -120, AMB_COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )
		...
		for k, v in pairs( AmbFactory2Chunks ) do
			...
			draw.SimpleTextOutlined( self:GetNWInt('Workpiece_'..k), 'ambFont22', -400, -90 + 26 * k, self:ColorCondition( k ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )

		end

		if IsDelay() then

			draw.SimpleTextOutlined( 'Подождите '..GetDelay()..' секунд', 'ambFont18', -150, -26, AMB_COLOR_RED, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )

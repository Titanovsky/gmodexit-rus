...
function ENT:DrawTranslucent()
	...
	if imgui.Entity3D2D( self, Vector( 19, 0, 32 ), Angle( 0, 60, 90 ), 0.1, 600 ) then
		...
		draw.SimpleTextOutlined( 'Склад Металла', 'ambFont32', -110, -230, AMB_COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )
		draw.SimpleTextOutlined( 'Состояние: ', 'ambFont22', -184, -180, AMB_COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )
		draw.SimpleTextOutlined( self:GetNWInt( 'Metal' )..' ед.', 'ambFont22', -136, -180, condition_color, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )
		...
		if ( timer.Exists( 'AmbTimeFac2TakeMetall'..LocalPlayer():SteamID() ) ) then
			...
			draw.SimpleTextOutlined( 'Подождите '..math.Round( timer.TimeLeft( 'AmbTimeFac2TakeMetall'..LocalPlayer():SteamID() ) )..' seconds', 'ambFont18', -234, -150, AMB_COLOR_RED, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLACK )

		end

		if imgui.xTextButton( 'Взять', font, -160, -120, 100, 42, 2, AMB_COLOR_GREEN, COLOR_GREEN, AMB_COLOR_GREEN ) and ( LocalPlayer():GetPos():Distance( self:GetPos() ) <= 82 ) and ( self:GetNWInt( 'Metal' ) > 0 ) and ( timer.Exists( 'AmbTimeFac2TakeMetall'..LocalPlayer():SteamID() ) == false ) then

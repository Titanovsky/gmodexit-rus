...
local COLOR_BAIT = Color( 138, 255, 183 )
...
function ENT:Touch( eEntity )
	if ( eEntity:GetClass() == "amb_fishing_hook" ) then
		self.hook = eEntity
		eEntity:SetColor( Color( 255, 255, 255, 0 ) )
...
function ENT:Think()
	if ( self:WaterLevel() > 2 ) and ( self:GetNWBool("OnLoad") ) then
		if not timer.Exists( "AmbFishing[timer]"..self:EntIndex() ) then -- это обязательно иначе бум бум будте
			...
			timer.Create( "AmbFishing[timer]"..self:EntIndex(), math.random( 4, 12 ), 1, function()
				if IsValid( self ) and ( self:WaterLevel() > 2 ) and ( self:GetNWBool("OnLoad") ) then
					_hook:SetNWBool( "OnCatch", true )
					_hook:TakeCatch()
					self:Remove()
				end
				_hook:SetColor( Color( 0, 255, 0 ) )
				_hook:SetNWBool( "OnBusy", false ) 
			end )
		end
	end
end

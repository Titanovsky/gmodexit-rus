...
local black_list_ents = { 

	[ 'ent_jack_gmod_ezaidradio' ] = true,
	[ 'ent_jack_gmod_ezfieldhospital' ] = true,

}

local function IsAllowed(Player, Class, EntityClass)

	if AmbRestrict.block_e2[ Player:SteamID() ] then Player:ChatPrint('Тебе дураку ограничили E2 и Adv.Dupe2 доступ!') return false end
	...
	for k, v in pairs( AmbRestrict.entities_blacklist ) do

		if ( Class == v ) and not Player:IsSuperAdmin() then return false end

	end

	if AmbEconomic.EntitiesList[ Class ] then

		local summ = AmbEconomic.EntitiesList[ Class ][ 2 ]
		local lvl = AmbEconomic.EntitiesList[ Class ][ 1 ]

		if ( tonumber( Player:GetNWInt('amb_players_money') ) < summ ) then

			AmbLib.notifySend( Player, 'Не хватает Био.Эссенций '..summ..' BE', 1, 3, 'buttons/button10.wav' )

			return false

		end

		if ( tonumber( Player:GetNWInt('amb_players_level') ) < lvl ) then

			AmbLib.notifySend( Player, 'Нужен уровень '..lvl, 1, 3, 'buttons/button10.wav' )
